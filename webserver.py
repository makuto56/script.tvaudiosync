...
def get_state():
    """Snapshot thread-safe de l'etat courant (lu depuis le thread
    principal de l'addon ET depuis le thread du serveur HTTP)."""
    with _STATE_LOCK:
        return dict(STATE)
...
function renderPr(){
 ...
 presets.forEach(function(p){
  ...
  d.innerHTML='<span><b>'+esc(p.name)+'</b><br><small style="color:#888">'+esc(p.channel)+' &rarr; '+esc(p.audio)+' ('+esc(p.offset)+'ms, '+esc(p.volume)+'%)</small></span>';
  var b1=document.createElement('button');b1.textContent='\u25B6';b1.onclick=function(){cmd('load_preset',p.id);};
  var b2=document.createElement('button');b2.textContent='\\uD83D\\uDDD1';b2.className='d';b2.onclick=function(){cmd('delete_preset',p.id);setTimeout(refreshPr,400);};
...
LOGIN_HTML = u"""<!DOCTYPE html><html><head><meta charset="utf-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>TV Audio Sync</title>
<style>body{font-family:system-ui,sans-serif;background:#1a1a1a;color:#e0e0e0;
padding:32px;max-width:520px;margin:auto}code{background:#2a2a2a;padding:2px 6px;
border-radius:4px}</style></head><body>
<h2>&#128274; Acces protege</h2>
<p>Un jeton d'acces est configure pour cette interface. Ajoutez-le a l'URL :</p>
<p><code>http://IP_DE_LA_BOX:8090/?token=VOTRE_JETON</code></p>
<p>Le jeton se trouve dans les parametres de l'addon Kodi (TV Audio Sync &rarr;
Configurer &rarr; Interface web).</p>
</body></html>"""
...
def _presets_load():
    ...
    try:
        ...
        return []
    # Migration douce : les presets crees avant l'ajout de l'id stable
    # recoivent un id base sur leur position, pour rester utilisables.
    changed = False
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _authorized(self):
        """Verifie le jeton d'acces si l'admin en a configure un dans les
        parametres de l'addon. Accepte soit l'en-tete X-Auth-Token (utilise
        par le JS une fois la page chargee), soit ?token= dans l'URL (utile
    ...
    def do_GET(self):
        ...
        if p == '/api/log':
            try:
                n = int(_qs_get(self.path, 'lines', '50'))
            except ValueError:
                n = 50
            lines = []
    ...
    def do_POST(self):
        ...
        if a == 'save_preset':
            ...
            ps.append({
                ...
                'offset': snap.get('offset', 0),
                'volume': snap.get('volume', 100),
            })
            _presets_save(ps)
            return self._json({'ok': True})
        if a == 'delete_preset':
            ps = _presets_load()
            ps = [x for x in ps if str(x.get('id')) != str(v)]
            _presets_save(ps)
            return self._json({'ok': True})
        if a == 'load_preset':
            preset = None
            for x in _presets_load():
                if str(x.get('id')) == str(v):
                    preset = x
                    break
            if preset is None:
...
def start_server(command_queue, presets_file, log_file, channels, audios, port=WEB_PORT, web_token=''):
    global COMMAND_QUEUE, PRESETS_FILE, LOG_FILE, M3U_CHANNELS, M3U_AUDIOS, WEB_TOKEN, _SERVER
